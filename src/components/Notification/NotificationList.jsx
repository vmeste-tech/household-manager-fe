...
// Объект для сопоставления категорий с цветом фона (blue и indigo оттенки)
const colorMapping = {
  задачи: "bg-green-500",
...
const NotificationList = ({ notifications, onSelect }) => {
  return (
    <ul role="list" className="divide-y divide-gray-100 m-0 p-0 list-none">
      {notifications.map((notification) => {
        // Форматирование даты в виде "2 мар 23:34"
        ...
        // Выбираем SVG-иконку в зависимости от категории
        const icon =
          iconMapping[notification.category.toLowerCase()] || defaultIcon;

        // Выбираем цвет фона в зависимости от категории
        const bgColor =
        ...
        return (
          <li
            key={notification.id}
            className="flex justify-between gap-x-6 cursor-pointer hover:bg-gray-100 p-3"
            onClick={() => onSelect(notification)}
          >
            <div className="flex items-center gap-x-4">
              <div
                className={`h-12 w-12 flex-none rounded-full ${bgColor} flex items-center justify-center p-2 text-white`}
              >
                {icon}
              </div>
              {/* Индикатор непрочитанности */}
              <div
                className={`w-2 h-2 rounded-full mx-2 transition-all duration-200 ${
                  notification.read ? "bg-transparent" : "bg-blue-500"
                }`}
              />

              <div className="flex flex-col justify-between min-h-12 min-w-0">
                <p className="font-semibold text-gray-900 mb-0">
                  {notification.category}
                </p>
                <p className="text-gray-500 mb-0">{notification.description}</p>
              </div>
            </div>
